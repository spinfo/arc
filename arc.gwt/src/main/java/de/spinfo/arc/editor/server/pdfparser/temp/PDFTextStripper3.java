 ...
 *
 */
package de.spinfo.arc.editor.server.pdfparser.temp;
 ...
 * 
 */
public class PDFTextStripper3 extends PDFStreamEngine
{

    private static final String thisClassName = PDFTextStripper3.class.getSimpleName().toLowerCase();
     ...
     */
    public PDFTextStripper3( Properties props ) throws IOException
    {
        super( props );
        this.outputEncoding = null;
        normalize = new TextNormalize(this.outputEncoding);
     ...
     * @throws IOException If there is an error reading the properties.
     */
    public PDFTextStripper3( String encoding ) throws IOException
    {
        super( ResourceLoader.loadProperties(
                "org/apache/pdfbox/resources/PDFTextStripper.properties", true ));
                        ...
                        // David
   						...
   						if (!line.isEmpty()) {
   							...
   							for (TextPosition tp : line) {
   								if (tp != null) 
   									// Strange sometime getCharacter yields null what is supposed to be a whitespace
   									if (tp.getCharacter() == null)
   										buffer += " ";
   									else
   										buffer += tp.getCharacter();
//   									getU().extractTextFeatures(tp, getCurrentPageNo());
   							}
