package de.uni_koeln.spinfo.arc.editor.server.persistance.service.update;
...
public class WordUpdater {
	...
	public synchronized WriteResult pushForm(long index,
		...
		return mongoTemplate.updateFirst(
				...
				new Update().push(WORD_FORM_STACK_FIELD, fromAnnoToPush),
				wordCollectionName);
	}
	...
	public synchronized WriteResult popLastForm(long index) {
		return mongoTemplate.updateFirst(
				Query.query(Criteria.where("index").is(index)),
				new Update().pop(WORD_FORM_STACK_FIELD, Position.LAST),
				wordCollectionName);
	}
	...
	public synchronized WriteResult pushRect(long index,
		...
		return mongoTemplate.updateFirst(
				Query.query(Criteria.where("index").is(index)),
				new Update().push(WORD_RECT_STACK_FIELD, rectAnnoToPush),
				wordCollectionName);
	}
	...
	public synchronized WriteResult popLastRect(long index) {
		return mongoTemplate.updateFirst(
				Query.query(Criteria.where("index").is(index)),
				new Update().pop(WORD_RECT_STACK_FIELD, Position.LAST),
				wordCollectionName);
	}
	...
	public synchronized WriteResult popLastPos(long index) {
		return mongoTemplate.updateFirst(
				Query.query(Criteria.where("index").is(index)),
				new Update().pop(WORD_POS_STACK_FIELD, Position.LAST),
				wordCollectionName);
	}
	...
	public WriteResult pushPosTaggerOptions(long index, Set<PosTags> posOptions) {
		return mongoTemplate.updateFirst(Query.query(Criteria.where("index")
				.is(index)), new Update().addToSet(WORD_POS_OPTIONS_FIELD)
				.each(posOptions), wordCollectionName);
	}

	public synchronized WriteResult pushPosTaggerOption(long index,
			PosTags posOption) {
		return mongoTemplate.updateFirst(
				Query.query(Criteria.where("index").is(index)),
				new Update().addToSet(WORD_POS_OPTIONS_FIELD, posOption),
				wordCollectionName);
	}

	public WriteResult pushPosTaggerOptionsAsStrings(long index,
			Set<String> posOptions) {
		if (index < 0 || posOptions.isEmpty())
			throw new IllegalArgumentException(
					"index must not be less than zero and/or pos option must not be an empty ssettring!");
		return mongoTemplate.updateFirst(Query.query(Criteria.where("index")
				.is(index)), new Update().addToSet(WORD_POS_OPTIONS_FIELD)
				.each(posOptions), wordCollectionName);
	}
	...
	public synchronized WriteResult pushPosTaggerOption(long index,
		...
		if (index < 0 || posOption.isEmpty())
			throw new IllegalArgumentException(
					"index must not be less than zero and/or pos option msut not be an empty string!");
		return mongoTemplate.updateFirst(
				Query.query(Criteria.where("index").is(index)),
				new Update().addToSet(WORD_POS_OPTIONS_FIELD, posOption),
				wordCollectionName);
	...
	/**
	 * In order to fill the "_class"-Field of the object to serialized this
	 * method needs to be called before serializing the object. This is a
	 * workaround otherwise queries with springdata against mongoDb wouldn't
	 * work
	 * 
	 * @param annotation
