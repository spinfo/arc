...
public class WordsContainerImpl extends Composite {
	...
	private List<WordDto> words = new LinkedList<>();

	public WordsContainerImpl(int pageNum, List<WordDto> words) {
	...
	private void setupimage() {
		...
		img.addLoadHandler(new LoadHandler() {
			...
			public void onLoad(LoadEvent event) {
				...
				pageImage = face;
				context2d.drawImage(pageImage, 0, 0, face.getWidth(),
						face.getHeight());
				...
				KeyUpHandler enterHandler = new KeyUpHandler() {
					...
					public void onKeyUp(KeyUpEvent event) {
						...
						if (lastSelectedWord != null) {
							...
							scaleWidth = Double.parseDouble(widthScaleInput
									.getText());
							scaleHeight = Double.parseDouble(heightScaleInput
									.getText());
							drawWord(lastSelectedWord);
						}
						...
						if (isEditCoordinatsMode) {
							...
							isRightEdgeClicked = x < (currentX + currentWidth + GRAB_TOLERANCE)
									...
									&& (y >= currentY && y <= currentY
											+ currentHeight);
							System.out.println("is right edge clicked? "
									+ isRightEdgeClicked);

							isTopEdgeClicked = (y > (currentY - GRAB_TOLERANCE) && y <= currentY)
									&& (x >= currentX && x <= currentX
											+ currentWidth);
							System.out.println("is top edge clicked? "
									+ isTopEdgeClicked);
							...
							isBottomEdgeClicked = y < (currentY + currentHeight + GRAB_TOLERANCE)
									...
									&& (x >= currentX && x <= currentX
											+ currentWidth);
							System.out.println("is bottom edge clicked? "
									+ isBottomEdgeClicked);

							isUpperLeftClicked = (y > (currentY - GRAB_TOLERANCE) && y <= currentY)
									&& (x >= currentX - GRAB_TOLERANCE && x <= currentX);
							System.out.println("is isUpperLeftClicked? "
									+ isUpperLeftClicked);
							...
							System.out.println("is isUpperRightClicked? "
									+ isUpperRightClicked);
							...
							isLowerLeftClicked = y < (currentY + currentHeight + GRAB_TOLERANCE)
									&& y >= (currentY + currentHeight)
									&& (x >= currentX - GRAB_TOLERANCE && x <= currentX);
							System.out.println("is isLowerLeftClicked? "
									+ isLowerLeftClicked);
							...
							isLowerRightClicked = y < (currentY + currentHeight + GRAB_TOLERANCE)
									...
									&& x >= (currentX + currentWidth)
									&& x < (currentX + currentWidth + GRAB_TOLERANCE);
							System.out.println("is isLowerRightClicked? "
									+ isLowerRightClicked);
					...
					public void onMouseMove(MouseMoveEvent event) {
						if (isEditCoordinatsMode) {
							...
							if (isInRectClicked) {
								...
								calcW = currentWidth;
								calcH = currentHeight;
								System.out.println("in rect! : x:" + newX
										+ "y: " + newY);
								// drawCurrentRect(newX , newY, currentWidth,
								// currentHeight);
							}
							// else
							if (isLeftEdgeClicked) {
								...
								calcW = newWidth;
								calcH = currentHeight;
								// drawCurrentRect(newX , currentY, newWidth,
								// currentHeight);
								System.out
										.println("moving left edge. new Rect: x: "
												+ newX
												+ " y: "
												+ y
												+ "width: "
							...
							if (isRightEdgeClicked) {
								int distToRightEdge = x
										- (currentX + currentWidth);
								int newWidth = currentWidth + distToRightEdge;
								System.out.println("Dist To Right edge: "
										+ distToRightEdge);
								...
								calcW = newWidth;
								calcH = currentHeight;
								// drawCurrentRect(currentX , currentY,
								// newWidth, currentHeight);
								// System.out.println("moving right edge. new Rect: x: "
								// + x + " y: " + y + "width: " + newWidth +
							...
							System.out.println("MouseMove @ x: " + x + " y: "
									+ y);
	...
	private WordDto lastSelectedWord = null;
	...
	private void drawCurrentRect(int x, int y, int width, int height) {
		context2dToDraw.clearRect(0, 0,
				canvasToDrawOn.getCoordinateSpaceWidth(),
				canvasToDrawOn.getCoordinateSpaceHeight());
		if (pageImage != null)
			context2d.drawImage(pageImage, 0, 0, pageImage.getWidth(),
					pageImage.getHeight());
		if (isEditCoordinatsMode) {
			 ...
			 */
			context2dToDraw.setFillStyle(CssColor.make("rgba(" + 255 + ", "
					+ 255 + "," + 255 + ", " + 0.8 + ")"));
			// grey left rect
			...
			// right from word
			context2dToDraw.fillRect(x + width, 0, pageImage.getWidth()
					- (x + width), pageImage.getHeight());
			// bottom of word
			context2dToDraw.fillRect(x, y + height, width,
					pageImage.getHeight() - (y + height));

			/*
			 * draw the drag bars
			 */
			context2dToDraw.setFillStyle(CssColor.make("rgba(" + 0 + ", " + 0
					+ "," + 255 + ", " + 0.1 + ")"));
			context2dToDraw.setStrokeStyle(CssColor.make("rgba(" + 0 + ", " + 0
					+ "," + 255 + ", " + 0.5 + ")"));
			// left drag bar
			// context2dToDraw.fillRect(x - GRAB_TOLERANCE, y, GRAB_TOLERANCE,
			// height);
			context2dToDraw.strokeRect(x - GRAB_TOLERANCE, y, GRAB_TOLERANCE,
					height);
			// right drag bar
			// context2dToDraw.fillRect(x + width, y, GRAB_TOLERANCE, height);
			context2dToDraw.strokeRect(x + width, y, GRAB_TOLERANCE, height);
			// top drag bar
			// context2dToDraw.fillRect(x, y - GRAB_TOLERANCE, width,
			// GRAB_TOLERANCE);
			context2dToDraw.strokeRect(x, y - GRAB_TOLERANCE, width,
					GRAB_TOLERANCE);
			// bottom drag bar
			...
			/*
			 * Draw diagonal scale handle drag bars (upper left, upper right,
			...
			// upper left
			context2dToDraw.fillRect(x - GRAB_TOLERANCE, y - GRAB_TOLERANCE,
					GRAB_TOLERANCE, GRAB_TOLERANCE);
			// upper right
			context2dToDraw.fillRect(x + width, y - GRAB_TOLERANCE,
					GRAB_TOLERANCE, GRAB_TOLERANCE);
			// lower left
			context2dToDraw.fillRect(x - GRAB_TOLERANCE, y + height,
					GRAB_TOLERANCE, GRAB_TOLERANCE);
			// lower right
			context2dToDraw.fillRect(x + width, y + height, GRAB_TOLERANCE,
					GRAB_TOLERANCE);
			...
			context2dToDraw.moveTo(x + width + GRAB_TOLERANCE, y
					+ (height * 0.5));
			...
			context2dToDraw.moveTo(x + (width * 0.5), y + height
					+ GRAB_TOLERANCE);
	...
	private void updateSelectedCoordinates() {
		RectangleAnnotationDto rectanno = new RectangleAnnotationDtoImpl(
				new Date(), 0, "admin", currentX, currentY, currentWidth,
				currentHeight);
		lastSelectedWord.setAnnotationAsType(AnnotationTypes.RECTANGLE,
				rectanno);

	}

	private void drawWord(WordDto word) {
		if (pageImage != null) {
			RectangleAnnotationDto rect = word.getLastRectangleAnnotation();
			// context2dToDraw.clearRect(0 , 0 ,
			// canvasToDrawOn.getCoordinateSpaceWidth() ,
			// canvasToDrawOn.getCoordinateSpaceHeight() );
			...
			context2d.drawImage(pageImage, 0, 0, pageImage.getWidth(),
					pageImage.getHeight());
			// context2dToDraw.fillRect(0, 0, pageImage.getWidth(),
			// pageImage.getHeight());
	...
	private void drawWordBasedOnAverageFontSize() {
		if (lastSelectedWord != null) {
			...
			RectangleAnnotationDto rectAnno = lastSelectedWord
					.getLastRectangleAnnotation();
			FormAnnotationDto str = lastSelectedWord.getLastFormAnnotation();

			context2dToDraw.clearRect(0, 0,
					canvasToDrawOn.getCoordinateSpaceWidth(),
					canvasToDrawOn.getCoordinateSpaceHeight());
			double x = (double) (rectAnno.getX() * scaleX);
			double y = (double) (rectAnno.getY() * scaleY);

			double w = (str.getForm().length() * scaleWidth);
	...
	private static class MyDialogBox extends DialogBox {
		...
		public MyDialogBox(ClickHandler closeHandler,
				ClickHandler editCoordinatesHandler) {
			// PopupPanel's constructor takes 'auto-hide' as its boolean
			// parameter.
			// If this is set, the panel closes itself automatically when the
			// user
	...
	public void onWordRightClicked(WordWidgetImpl wordWidgetImpl,
		...
		if (!isWordRightClicked) {
			// popup.wordIndexLabel.setText("SELECTED: <" +
			// wordWidgetImpl.getText() + "> index: [" +
			// wordWidgetImpl.getIndex() + "]");
			// popup.setPopupPosition(event.getNativeEvent().getClientX(),
			// event.getNativeEvent().getClientY());
			// popup.show();
			dialogBox.wordIndexLabel.setText("SELECTED: <"
					+ wordWidgetImpl.getText() + "> index: ["
					+ wordWidgetImpl.getIndex() + "]");
			dialogBox.setPopupPosition(event.getNativeEvent().getClientX(),
					event.getNativeEvent().getClientY());
			dialogBox.show();
