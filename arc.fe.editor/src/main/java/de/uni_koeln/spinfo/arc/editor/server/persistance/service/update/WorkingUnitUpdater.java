...
public class WorkingUnitUpdater {
	...
	public WorkingUnitUpdater(ApplicationContext ctx,
			String workingUnitCollectionName) {
		super();
		this.ctx = ctx;
	...
	public WriteResult pushChapter(String workingUnitTitle,
			ChapterRange chapterRange) {
		nullCheck(chapterRange);
		DBObject chapterRangeToPush = setUpForClassSerialization(chapterRange);
		return mongoTemplate.updateFirst(
				Query.query(Criteria.where("title").is(workingUnitTitle)),
				new Update().push(CHAPTER_STACK_FIELD, chapterRangeToPush),
				workingUnitCollectionName);
	}

	// below is the mongoDB cmd for pulling a chapter
	// db.workingUnits.update( { }, { $pull: {"annotations.CHAPTER_RANGE" :
	// {"title":"testTitle"} } }, {multi: true} )

	public synchronized WriteResult pullChapter(String workingUnitTitle,
			ChapterRange chapterRange) {
		nullCheck(chapterRange);
		DBObject chapterRangeToPull = setUpForClassSerialization(chapterRange);
		// Below is how its done more less without spring
		// DBCollection coll =
		// mongoTemplate.getCollection(workingUnitCollectionName);
		// BasicDBObject match = new BasicDBObject("title", workingUnitTitle);
		// BasicDBObject update = new BasicDBObject("annotations.CHAPTER_RANGE",
		// chapterRangeToPull);
		// return coll.update(match, new BasicDBObject("$pull", update), false,
		// false);
		return mongoTemplate.updateFirst(
				Query.query(Criteria.where("title").is(workingUnitTitle)),
				new Update().pull(CHAPTER_STACK_FIELD, chapterRangeToPull),
	...
	public synchronized WriteResult pushLanguage(String workingUnitTitle,
			LanguageRange languageRange) {
		nullCheck(languageRange);
		DBObject languageRangeToPush = setUpForClassSerialization(languageRange);
		return mongoTemplate.updateFirst(
				Query.query(Criteria.where("title").is(workingUnitTitle)),
				new Update().push(LANGUAGE_STACK_FIELD, languageRangeToPush),
				workingUnitCollectionName);
	}

	public synchronized WriteResult pullLanguage(String workingUnitTitle,
			LanguageRange languageRange) {
		nullCheck(languageRange);
		DBObject chapterRangeToPull = setUpForClassSerialization(languageRange);
		return mongoTemplate.updateFirst(
				Query.query(Criteria.where("title").is(workingUnitTitle)),
				new Update().pull(LANGUAGE_STACK_FIELD, chapterRangeToPull),
				WorkingUnitImpl.class);
	}
