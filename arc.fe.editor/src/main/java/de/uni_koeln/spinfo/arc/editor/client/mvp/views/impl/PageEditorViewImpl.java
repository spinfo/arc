...
//	    Image img = new Image("crestoJpg.jpg");
	    ...
	    img.addLoadListener(new LoadListener() {
			...
			ErrorIndicator.busy("Error!", "Cannot load the image for this Page!", "Click next to this popup in order to close this message");
			}
		});
...
public void drawWord(int x, int y, int w, int h, CssColor wordSelectionFrameColor ) {	
	...
	if (x - 50 >= 0)
		imageScrollPanel.setHorizontalScrollPosition(x - 50);
	else
		imageScrollPanel.setHorizontalScrollPosition(0);
}

@Override
public ScrollPanel getWordsScrollPanel() {
	return wordsScrollPanel;
}
...
@Override
public void setModeInfoText(String text) {
