...
public class ConvertToNewModel implements ChrestomatieConst {
	...
	public static void main(String[] args) throws Exception {
		if (args != null)
			if (args.length > 0) {
				...
				System.out
						.println("< No Args for Conversion supplied > Taking just the first Form Modification (HEURISTIC 2)");
	...
	private static void setUpSpring() {
		...
		// workingUniImplDaoRepository =
		// ctx.getBean(WorkingUnitImplDaoRepository.class);
	...
	private static void convert(String[] relevantUserIds) {
		...
		System.out
				.println("You'll get spare progress infos... consider grapping a coffee.. ");
		...
		for (int i = 0; i < wuTitles.length; i++) {
			// for (int i = 0; i < 1; i++) {
			int WU_IDX = i;
			System.out.println("converting Working Unit #" + WU_IDX + " / "
					+ wuTitles.length + "\n Title: " + wuTitles[WU_IDX]);
			String WU_VAL_TO_QUERY = wuTitles[WU_IDX]; // 2722117 - 2775612
			WorkingUnit inMemoryWorkingUnit = rwua
					.getWorkingUnit(WU_VAL_TO_QUERY);
		...
		System.out
				.println("=====================================================================");
		System.out
				.println("DONE WITH FULL CONVERSION! Fire up the arc editor and behold the changes!");
		System.out
				.println("---------------------------------------------------------------------");
	}
	...
	private static void dropPrevious() throws Exception {
		...
		if (mongoOperations.getCollection(WORKING_UNIT_COLLECTION_UNDER_TEST)
				.count() > 0) {
			System.err
					.println("Collection: "
							+ WORKING_UNIT_COLLECTION_UNDER_TEST
							+ " couldn't be droped before converting! Please drop it and convert again!");
			throw new Exception();
		} else
